VYOMA Complete Fixed app.js
Complete JavaScript source with the integrated voice policy.
...
if(typeof firebase === "undefined" || !window.VYOMA_FIREBASE_CONFIG || !window.VYOMA_FIREBASE_CONFIG.apiKey || window.VYOMA_
...
if (typeof window.vyomaProcessExtendedImu === "function") window.vyomaProcessExtendedImu({ ax, ay, az, gx, gy, gz, timestamp
: now });
...
if (note) {
note.textContent =
"This measures browser loop throughput only. It does not prove 10 Hz sensor navigation accuracy or 200 Hz external-IMU
...
if(data.drift !== undefined) set("aiDrDrift", typeof data.drift === "number" ? `${Number(data.drift).toFixed(1)}%` : data.
drift);
...
set("vyomaExtendedNote", "Stationary IMU baseline saved. Keep the phone mounted consistently; this is a baseline calibrati
on, not a full vehicle-frame alignment proof.");
...
const text = `VYOMA status. Road condition ${state.roadCondition}. Vibration score ${state.vibrationScore.toFixed(2)}. ${n
avigator.onLine ? "Online" : "Offline"}.`;
...
if(!text) return;
const destinationMatch = text.match(/(?:set|choose|select|go|navigate|take me)\s+(?:the\s+)?(?:destination|to)?\s*(?:to|at
)?\s+(.+)/i) ||
...
/\b(turn\s+(left|right)|continue\s+(straight|ahead)|go\s+straight|keep\s+(left|right)|bear\s+(left|right)|slight\s+(left|r
ight)|sharp\s+(left|right)|make\s+a\s+(left|right)\s+turn|enter\s+(the\s+)?roundabout|take\s+the\s+(first|second|third|fourth)
\s+exit|merge)\b/i;
const blockedPattern =
/\b(destination|coordinates?|latitude|longitude|calculate|calculating|route|navigation|navigating|not\s+navigating|ready|s
tarted|starting|stopped|recalculating|searching|dead\s+reckoning|current\s+location|pothole|bump|arrival|arrived|arrive|voice|
guidance|assistance|status|command|understood|enabled|disabled|system)\b/i;
