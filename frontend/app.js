...
(function initVyomaFirebaseLogin(){
 ...
 if(typeof firebase === "undefined" || !window.VYOMA_FIREBASE_CONFIG || !window.VYOMA_FIREBASE_CONFIG.apiKey || window.VYOMA_FIREBASE_CONFIG loginError.textContent="Firebase is not configured. Add your Firebase Web App config in firebase-config.js.";
 return;
 }
 ...
 */
 ...
 if (note) {
 note.textContent =
 "This measures browser loop throughput only. It does not prove 10 Hz sensor navigation accuracy or 200 Hz external-IMU hardware perfo }
 };
...
(function () {
 ...
 set("vyomaExtendedNote", "Stationary IMU baseline saved. Keep the phone mounted consistently; this is a baseline calibration, not a full v recordEvent("calibration", state.calibration);
 }
 ...
 const text = `VYOMA status. Road condition ${state.roadCondition}. Vibration score ${state.vibrationScore.toFixed(2)}. ${navigator.onLine  if ("speechSynthesis" in window) {
...
(function initVyomaVoiceAssistant(){
 ...
 // Spoken status changes for important navigation events.
 // The observer keeps alerts connected to the existing UI without changing
 // the navigation logic or speaking every intermediate DOM mutation.
 const observedStatusNodes = [
 document.getElementById("systemStatus"),
 document.getElementById("navStatus"),
 document.getElementById("routeStatus")
 ].filter(Boolean);
 let lastSpokenStatus = "";
 let statusSpeakTimer = null;
 const statusObserver = new MutationObserver(() => {
 clearTimeout(statusSpeakTimer);
 statusSpeakTimer = setTimeout(() => {
 const text = observedStatusNodes.map(node => node.textContent.trim()).filter(Boolean).join(". ");
 if(!text || text === lastSpokenStatus) return;
 lastSpokenStatus = text;
 // Only speak concise, meaningful alerts; avoid repeating the same text.
 if(text.length <= 180 && /lost|restored|route|navigation|destination|error|calculating|arrive|ready/i.test(text)){
 setTranscript(text);
 window.vyomaSpeak(text);
 }
 }, 650);
 });
 observedStatusNodes.forEach(node => statusObserver.observe(node, {childList:true, characterData:true, subtree:true}));
 const originalStatusDescriptor = window.vyomaSetSpokenStatus;
 window.vyomaSetSpokenStatus = function(text){
 setTranscript(text);
 window.vyomaSpeak(text);
 if(typeof originalStatusDescriptor === "function") originalStatusDescriptor(text);
 };
})();
/* =========================================================
 ABOUT US POPUP
...
========================================================= */
(function () {
 const allowedGuidance = /\b(turn left|turn right|continue straight|continue ahead|keep left|keep right|enter roundabout|merge|arrive at des function isVoiceEnabled() {
 ...
 if (/calculating route|calculating navigation|route calculated|navigation started|navigation ready|recalculating|searching for route|rout return false;
 }
...
/* VYOMA FINAL EXACT VOICE POLICY
 Voice is silent unless the voice-assistance toggle is ON.
 Only maneuver directions and GNSS lost/restored are spoken. */
(function installExactVoicePolicy(){
 const synth = window.speechSynthesis;
 if (!synth || window.__vyomaExactVoicePolicyInstalled) return;
 window.__vyomaExactVoicePolicyInstalled = true;
 function enabled(){
 const t = document.getElementById('settingsVoiceToggle') ||
 document.getElementById('voiceGuidanceToggle');
 return !!(t && t.checked);
 }
 const allowed = /\b(turn left|turn right|keep left|keep right|continue straight|continue ahead|go straight|enter roundabout|take (the )?(fi const forbidden = /(voice guidance|destination|calculate route|route (calculated|ready|found)|not navigating|navigation (started|ready|activ const original = synth.speak.bind(synth);
 synth.speak = function(utterance){
 const text = String(utterance && utterance.text || '').trim();
 if (!enabled() || !text || forbidden.test(text) || !allowed.test(text)) return;
 return original(utterance);
 };
 const originalSpeak = window.vyomaSpeak;
 window.vyomaSpeak = function(text){
 if (!enabled()) { synth.cancel(); return; }
 const value = String(text || '').trim();
 if (forbidden.test(value) || !allowed.test(value)) return;
 if (typeof originalSpeak === 'function') return originalSpeak.call(this, value);
 };
 const originalGuidance = window.vyomaSpeakGuidance;
 window.vyomaSpeakGuidance = function(text){
 if (!enabled()) return;
 const value = String(text || '').trim();
 if (forbidden.test(value) || !allowed.test(value)) return;
 if (typeof originalGuidance === 'function') return originalGuidance.call(this, value);
 };
 document.addEventListener('change', function(e){
 const t = e.target;
 if (t && (t.id === 'settingsVoiceToggle' || t.id === 'voiceGuidanceToggle') && !t.checked) {
 synth.cancel();
 }
 });
})();
